...
export async function POST(req: NextRequest) {
  ...
  if (!user) return apiError("账号或密码错误");
  if (user.status === "DISABLED") return apiError("账号已停用，请联系企业管理员");
  if (user.status === "LOCKED") return apiError("账号已锁定，请联系企业管理员");
  if (user.role !== "ENTERPRISE_ADMIN") return apiError("该账号不是企业管理员，请使用正确入口登录");

  const valid = await bcrypt.compare(password, user.passwordHash);
  if (!valid) return apiError("账号或密码错误");
